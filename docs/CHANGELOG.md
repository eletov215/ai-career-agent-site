...
## 2026-07-20 — Устойчивое обновление «Работы России»
- Синхронизация вакансий переведена на адаптивные порции: 10, затем 5 и 1 при сетевом тайм-ауте.
- Тайм-аут чтения API увеличен до 45 секунд, добавлены повторные попытки временных сетевых ошибок.
- Счётчики синхронизации обновляются после каждой сохранённой порции.
- `/trudvsem/status` возвращает цель, текущую позицию, процент выполнения, состояние очереди и worker.
- Интерфейс показывает живой прогресс и автоматически обновляет список после успешной синхронизации.
- Удалены скомпилированные `__pycache__` из архива проекта.
